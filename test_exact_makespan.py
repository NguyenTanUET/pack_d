#!/usr/bin/env python3
"""
Test script to check if makespan = 364 is feasible for Pack_d001.data
"""
...
def test_makespan_364(data_file, time_limit=1800):
    """
    Test if makespan = 364 is feasible for Pack_d001.data
    Uses extended time limit to thoroughly test feasibility
    """
    print(f"Testing makespan = 364 for {data_file}")
    ...
    try:
        ...
        # Create makespan and add constraint for 364
        makespan = max(end_of(t) for t in tasks)
        mdl.add(makespan <= 364)

        print("\nTesting makespan <= 364...")
        ...
        if res is not None and res.is_solution():
            print("✅ RESULT: Makespan = 364 is FEASIBLE!")
            ...
            print("❌ RESULT: Makespan = 364 is INFEASIBLE!")
...
def main():
    # Test file
    data_file = Path("data/Pack_d001.data")

    if not data_file.exists():
        print(f"Error: File {data_file} not found!")
        return

    print("RCPSP Makespan Test for Pack_d001.data")
    print("=" * 80)

    # Test 1: Check if makespan = 364 is feasible
    print("\n🔍 TEST 1: Testing if makespan = 364 is feasible")
    feasible_364, actual_makespan, time_364 = test_makespan_364(data_file, time_limit=1800)
    ...
    print(f"Test 1 - Makespan = 364 feasible: {'YES' if feasible_364 else 'NO'}")
    if feasible_364 and actual_makespan:
        print(f"  Actual makespan found: {actual_makespan}")
    print(f"  Time taken: {time_364:.2f} seconds")

    print(f"\nTest 2 - Optimal makespan: {optimal_makespan if optimal_found else 'NOT FOUND'}")
    print(f"  Time taken: {time_optimal:.2f} seconds")

    if feasible_364 and optimal_found:
        if optimal_makespan == 364:
            print("\n✅ CONCLUSION: Makespan = 364 is indeed optimal!")
        elif optimal_makespan > 364:
            print(f"\n⚠️  CONCLUSION: Something is wrong! Optimal ({optimal_makespan}) > 364")
        else:
            print(f"\n❌ CONCLUSION: Makespan = 364 is NOT optimal. True optimal = {optimal_makespan}")

    elif not feasible_364 and optimal_found:
        print(f"\n✅ CONCLUSION: Makespan = 364 is infeasible. True optimal = {optimal_makespan}")
